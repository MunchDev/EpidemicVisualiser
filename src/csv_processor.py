...
def parse_data(date):
	...
	Returnn format: Dictionary(country_name, List(confirmed, deaths, recovered, active))
	
	For countries with multiple entries (i.e US, Canada, Australia), their state and provincial entries
	will be merged under one single entry.
	"""
	...
	if path.isfile("../cache/" + filename):
		...
		f.close()
		print("[Debug] Retrieve data from cache {}".format(date))
		return data_set
