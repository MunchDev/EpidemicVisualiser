...
def _validate_scale(s):
    ...
    if s != "log" and s != "linear":
        stderr(
            "Expected value of scale is either 'log' or 'linear', but given {}".format(s))
        return False
    return True
...
def _transpose_plot(countries, date, timespan, scale, plot_type):
    for country in countries:
        ...
        if timespan > 30:
            plt.plot(x_data, array([x[plot_type]
                                    for x in data]), "-", label=country)
        else:
            plt.plot(x_data, array([x[plot_type]
                                    for x in data]), ".-", label=country)
    ylabel = "Number of " + ["confirmed cases", "deaths",
                             "recovered cases", "active cases"][plot_type]
    plt.yscale(scale)
...
def plot_tally(country, date, timespan, *args, **kwargs):
    ...
    if not multiple_countries:
        ...
        if "c" in plot_type:
            if timespan > 30:
                plt.plot(x_data, array([x[0]
                                        for x in data]), 'b-', label="Confirmed")
            else:
                plt.plot(x_data, array([x[0]
                                        for x in data]), '.b-', label="Confirmed")
            plot_enabled = True
        if "d" in plot_type:
            if timespan > 30:
                plt.plot(x_data, array([x[1]
                                        for x in data]), 'k-', label="Deaths")
            else:
                plt.plot(x_data, array([x[1]
                                        for x in data]), '.k-', label="Deaths")
            plot_enabled = True
        if "r" in plot_type:
            if timespan > 30:
                plt.plot(x_data, array([x[2]
                                        for x in data]), 'g-', label="Recovered")
            else:
                plt.plot(x_data, array([x[2]
                                        for x in data]), '.g-', label="Recovered")
            plot_enabled = True
        if "a" in plot_type:
            if timespan > 30:
                plt.plot(x_data, array([x[3]
                                        for x in data]), 'r-', label="Active")
            else:
                plt.plot(x_data, array([x[3]
                                        for x in data]), '.r-', label="Active")
            plot_enabled = True
        ...
        for i in range(n):
            result = plot_tally(
                country[i], date, timespan, scale=scale[i], plot_type=plot_type[i])
            if result == -1:
                return -1
        return 0
